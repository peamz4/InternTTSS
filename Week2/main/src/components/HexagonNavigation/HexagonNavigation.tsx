        ...
        <div className="flex justify-center gap-2 mt-[-36px] xl:mt-[-48px] ">
          {hexagons.slice(0, 4).map((hex, index) => (
            <Tooltip label={hex.label} key={index} withArrow>
              <Link href={hex.link}>
                <div
                  className="group relative w-32 h-40 xl:w-48 xl:h-56 border border-gray-500 rounded-[10%] flex flex-col justify-center items-center shadow-lg
                ...
                >
                  <Center>
                    <hex.icon
                      size={60} /* คงที่ ไม่เพิ่มขนาดใน xl และ 2xl */
                      className="text-gray-900 dark:text-gray-200 transition-colors group-hover:text-orange-500"
                    />
                    <span className="absolute bottom-2 text-gray-700 dark:text-gray-300 mb-8 xl:mb-12 font-semibold text-sm xl:text-lg group-hover:text-orange-500">
                      {hex.label}
                    </span>
                  </Center>
                </div>
            ...
            <Tooltip label={hex.label} key={index + 4} withArrow>
              <Link href={hex.link}>
                ...
                >
                  <Center>
                    <hex.icon
                      size={40} /* คงที่ ไม่เพิ่มขนาดใน xl และ 2xl */
                      className="text-gray-900 dark:text-gray-200 transition-colors group-hover:text-orange-500"
                    />
                    <span className="absolute bottom-2 text-gray-700 dark:text-gray-300 mb-8 xl:mb-12 font-semibold text-sm xl:text-lg group-hover:text-orange-500">
                      {hex.label}
